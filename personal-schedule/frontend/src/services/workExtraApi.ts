...
export async function createWorkExtra(payload: Partial<Omit<WorkExtra, 'id' | 'created_at'>>) {
  const response = await api.post<WorkExtra>('/work-extras', payload)
  return response.data
}

export async function updateWorkExtra(workExtraId: number, payload: Partial<Omit<WorkExtra, 'id' | 'created_at'>>) {
